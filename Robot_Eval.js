...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'stim/Description & Consent.png', 'path': 'stim/Description & Consent.png'},
    {'name': 'demdet_csv.xlsx', 'path': 'demdet_csv.xlsx'},
    {'name': 'iri_csv.xlsx', 'path': 'iri_csv.xlsx'},
    {'name': 'Stimuli/stim/instructions.png', 'path': 'Stimuli/stim/instructions.png'},
...
function consent_routineRoutineEachFrame() {
  return async function () {
    ...
    // *yes_btn* updates
    if (t >= 0 && yes_btn.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    // *no_btn* updates
    if (t >= 0 && no_btn.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
...
function instructions_routineRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    instructions.setImage('Stimuli/stim/instructions.png');
